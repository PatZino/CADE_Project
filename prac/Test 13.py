...
# ---------------------------------------------------------------------------------------------
    ...
    for p in range(max_gen):
        if DE[p] < CA_fit_list[p]:
            fitness_updated_cade[p] = DE[p]
            idx = fitness_updated_cade.index(DE[p])
            DE_popt[p] = DE_pop[idx]
        else:
            fitness_updated_cade[p] = CA_fit_list[p]
            idx = fitness_updated_cade.index(CA_fit_list[p])
            CA_popt[p] = CA_pop[idx]
    # print("fitness_updated_cade: \n", fitness_updated_cade)

    updated_cade_pop = np.concatenate((DE_popt, CA_popt), axis=0)
...
# ----------------------- loop in the cade algorithm -------------------------------------------
    ...
    while number_of_runs < 50:
        ...
        for p in range(max_gen):
            if DE[p] < CA_fit_list[p]:
                fitness_updated_cade[p] = DE[p]
                idx = list(fitness_updated_cade).index(DE[p])
                DE_popt[p] = DE_pop[idx]
            else:
                fitness_updated_cade[p] = CA_fit_list[p]
        ...
        updated_cade_pop = np.concatenate((CA_popt, DE_popt), axis=0)  # new cade population
